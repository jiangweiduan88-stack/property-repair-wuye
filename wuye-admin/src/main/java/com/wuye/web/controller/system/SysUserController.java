...
import com.wuye.system.service.ISysPostService;
...
public class SysUserController extends BaseController
{
    ...
    public AjaxResult importData(@RequestParam(name = "file", required = false) MultipartFile file,
            @RequestParam(name = "updateSupport", defaultValue = "false") boolean updateSupport,
            @RequestParam(name = "validateOnly", defaultValue = "false") boolean validateOnly) throws Exception
    {
        if (file == null || file.isEmpty())
        {
            return AjaxResult.error("请选择需要导入的用户文件");
        }
        ExcelUtil<SysUserImportRow> util = new ExcelUtil<SysUserImportRow>(SysUserImportRow.class);
        List<SysUserImportRow> userList = util.importExcel(file.getInputStream());
        String operName = getUsername();
        SysUserImportResult result = userService.importUser(userList, updateSupport, operName, validateOnly);
